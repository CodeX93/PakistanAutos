...
const SparePartDetailsModal = ({ buy }) => {
          ...
          {/* Purchaser Information */}
            ...
            <Card
              ...
              >
                ...
                <Grid container spacing={2}>
                  {[
                    { label: "Name", value: buy.purchaserDetails?.name || "N/A" },
                    { label: "CNIC", value: buy.purchaserDetails?.cnic || "N/A" },
                    { label: "Contact", value: buy.purchaserDetails?.contactNo || "N/A" },
                    { label: "Address", value: buy.purchaserDetails?.address || "N/A" },
                  ].map((field, index) => (
            ...
            <Card
              ...
              >
                ...
                <Grid container spacing={2}>
                  {[
                    { label: "Name", value: buy.trustedPerson.name || "N/A" },
                    { label: "CNIC", value: buy.trustedPerson.cnic || "N/A" },
                    { label: "Contact", value: buy.trustedPerson.phone || "N/A" },
                    { label: "Address", value: buy.trustedPerson.address || "N/A" },
                  ].map((field, index) => (
                    <Grid item xs={6} key={index}>
                      <Typography variant="subtitle2" color="textSecondary">
                        {field.label}
                    ...
                    <Box key={index}>
                      <Grid container spacing={2}>
                        ...
                        <Grid item xs={4}>
                          ...
                          <Typography>{`${product.category} - ${product.subCategory}`}</Typography>
                        </Grid>
                        <Grid item xs={4}>
                          <Typography variant="subtitle2" color="textSecondary">
                            Quantity
                        ...
                        <Grid item xs={4}>
                          ...
                          <Typography>{formatCurrency(product.unitSellingPrice)}</Typography>
                        </Grid>
                        <Grid item xs={4}>
                          <Typography variant="subtitle2" color="textSecondary">
                            Total
                          </Typography>
                          <Typography>{formatCurrency(product.total)}</Typography>
                        </Grid>
                      </Grid>
                      {index < buy.products.length - 1 && <Divider sx={{ my: 2 }} />}
                    </Box>
                  ))}
              </Card>
            </Grid>
            ...
            <Card
              ...
              >
                ...
                <Grid container spacing={2}>
                  {[
                    { label: "Total Amount", value: formatCurrency(buy.totalAmount) },
            ...
            {/* Payment History */}
            {buy.paymentsReceived && buy.paymentsReceived.length > 0 && (
              <Grid item xs={12} md={10}>
                <Card
                  sx={{
                    padding: 3,
...
const handleSubmit = () => {
  ...
  const addFooter = () => {
    ...
    addField("CNIC:", selectedBuy.clientDetails?.cnic || "N/A", 90);

    currentY += spacing;
    addField("Contact No:", selectedBuy.clientDetails?.contactNo || "N/A", 0);
    addField("Address:", selectedBuy.clientDetails?.address || "N/A",90);
    currentY += spacing * 2;
    checkPageBreak();

    // Trusted Person Details
    addSectionHeader("Trusted Person Details");
    addField("Name:", selectedBuy.trustedPerson?.name || "N/A");
    addField("CNIC:", selectedBuy.trustedPerson?.cnic || "N/A", 90);
    currentY += spacing;
    addField("Contact:", selectedBuy.trustedPerson?.phone || "N/A", 0);
    addField("Address:", selectedBuy.trustedPerson?.address || "N/A",90);
    currentY += spacing * 2;
    checkPageBreak();

    // Supplier Details
    addSectionHeader("Supplier Details");
    selectedBuy.products?.forEach((product) => {
    addField("Name:", product.supplier?.name || "N/A");
    addField("CNIC:", product.supplier?.cnic || "N/A", 90);
    currentY += spacing;
    addField("Contact:", product.supplier?.contact || "N/A", 0);
    addField("Address:", product.supplier?.address || "N/A",90);
    currentY += spacing * 2;
    checkPageBreak();
    });

    // Product Details Section
    addSectionHeader("Products Purchased");
    selectedBuy.products?.forEach((product) => {
      addField("Product Name:", product.productName || "N/A");
      addField("Type:", product.bikeType || "N/A", 90);
      currentY += spacing;
      addField("Category:", product.category || "N/A", 0);
    ...
    selectedBuy.products?.forEach((product) => {
    addField("Total Selling Price:", `₨${ product.totalPrice?.toLocaleString() || "N/A"}`);
    });
    addField("Pending Balance:", `₨${selectedBuy.pendingBalance?.toLocaleString() || "N/A"}`, 90);
    currentY += spacing * 2;
    checkPageBreak();
      ...
      <Dialog open={isConfirmDialogOpen} onClose={() => setIsConfirmDialogOpen(false)}>
        ...
        <DialogActions>
          ...
          <Button
            ...
            variant="contained"
            sx={{
              "&:hover": { bgcolor: "#1976d2", transform: "scale(1.05)", transition: "0.3s" },
            }}
          >
            Confirm
          </Button>
...
const SparePartCreditBuys = () => {
  ...
  const handleMakePayment = async (payment) => {
    ...
    try {
      const response = await fetch(`${url}/SparePartCreditBuy/${selectedBuy.id}`, {
        method: "PUT",
  ...
  const PaymentTable = ({ payments }) => (
    <TableContainer component={Paper}>
      <Table>
        <TableHead>
          <TableRow>
            <TableCell sx={{ fontWeight: 'bold', fontSize: '1.1rem', backgroundColor: theme.palette.grey[200] }}>Purchaser</TableCell>
        ...
        <TableBody>
          {(!payments || payments.length === 0) ? (
            ...
            payments.map((buy, index) => (
              <TableRow key={index}>
                <TableCell>{buy.purchaserDetails?.name}</TableCell>
                <TableCell>{formatCurrency(buy.pendingBalance)}</TableCell>
                <TableCell>
                  <Box sx={{ display: 'flex', justifyContent: 'center', gap: 1 }}>
                    <SparePartDetailsModal buy={buy} />
                    <Button
                      variant="contained"
                      color="primary"
...
export default SparePartCreditBuys;